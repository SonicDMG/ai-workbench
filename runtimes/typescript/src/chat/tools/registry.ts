...
const listKbs: AgentTool = {
	definition: {
		name: "list_kbs",
		description:
			"List the knowledge bases available in this workspace, returning each one's `knowledgeBaseId`, name, and description. Use this when the user asks what data sources exist, or to discover which `knowledgeBaseId` to pass to `search_kb` before searching.",
		parameters: {
			type: "object",
			properties: {},
...
const searchKb: AgentTool = {
	definition: {
		name: "search_kb",
		description:
			"Semantic search across the workspace's knowledge bases. Returns the top matching chunks with a short content preview. Use this when the user asks a content question that requires looking inside the documents (not just listing them). Pass `knowledgeBaseId` to scope to one KB — call `list_kbs` first if you don't know which knowledge bases exist; omit `knowledgeBaseId` to search every KB.",
		parameters: {
			type: "object",
			required: ["query"],
			properties: {
